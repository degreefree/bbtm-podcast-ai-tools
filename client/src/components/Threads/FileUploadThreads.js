...
const FileUploadThreads = ({
  ...
  const readFile = async (event) => {
    ...
    try {
      ...
      const response = await fetch("http://wwww.localhost:3000/threads", {
        method: "POST",
        headers: {
      ...
      setHooks(JSON.parse(data.hooks)[0]);
      setBullets(JSON.parse(data.bullets)[0]);
      setCTAs(JSON.parse(data.ctas)[0]);
    } catch (error) {
...
export default FileUploadThreads;
